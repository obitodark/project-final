"use client"

import { titleFont } from "@/config/fonts"
import { getRequest } from "@/utils/http"
import { PlusIcon } from "@radix-ui/react-icons"
import { useDispatch } from "react-redux"
import { openModal } from "@/store/Modal"
import { Button } from "@/components/ui/button"
import { DataTableCustom } from "@/components/custom/DataTableCustom"
import { Box } from "@/components/custom/Box"
import type { APIResponseSubcategories, Subcategory } from "@/interface"
import { columnsSubcategory } from "../column/ColumnTableSubcategory"
import { useQuery } from "@tanstack/react-query"

export const TableSubcategory = () => {


  const dispatch = useDispatch();
  const { data: subcategories = [] } = useQuery<Subcategory[]>({
    queryKey: ['subcategories'],
    queryFn: async () => {
      return (await getRequest<APIResponseSubcategories>("/subcategories")).data?.result || [];
    },
  });
  return (
    <Box className="p-2 sm:p-5 bg-white shadow-xl shadow-stone-200 rounded-xl mt-3">
      <Box className="flex justify-between">
        <h1 className={`${titleFont.className} antialiased font-bold text-xl `}>
          Lista Subcategorias
        </h1>
        <Button variant="ghost" className="mx-2 font-semibold"
          onClick={() => dispatch(openModal({ name: "modalSubcategory", data: undefined }))}
        >
          Nuevo
          <PlusIcon className="mx-2" />
        </Button>
      </Box>
      {subcategories && (
        <DataTableCustom columns={columnsSubcategory} data={subcategories} pageSize={5} />
      )}
    </Box>
  )
}

